...
export function renderDocPage(linkName: string): string {
  ...
  try {
    decodedLinkName = decodeURIComponent(linkName)
  } catch {
    decodedLinkName = linkName
  }
    ...
    ref.seeAlso && ref.seeAlso.length > 0
      ? `<div class="doc-page__section">
        ...
        <div class="doc-page__see-also">
          ${ref.seeAlso
            .map(name => {
              const { title, linkName: ln } = seeAlsoInfo(String(name), data)
              return `<a class="doc-page__see-also-link" href="${href(`/ref/${ln}`)}" onclick="event.preventDefault();Playground.navigate('/ref/${ln}')">${escapeHtml(title)}</a>`
            })
            .join(' ')}
        </div>
      </div>`
...
function renderFunctionVariants(
  name: string,
  ref: ReturnType<typeof isFunctionReference> extends true ? never : Parameters<typeof isFunctionReference>[0],
): string {
  if (!isFunctionReference(ref)) return ''
  const { args, variants } = ref

  const retType = Array.isArray(ref.returns.type) ? ref.returns.type.join(' | ') : String(ref.returns.type)

  const normalVariants = variants
    .map(v => {
      const params = v.argumentNames
        .map(argName => {
...
function renderCustomVariants(ref: Reference): string {
  ...
  return ref.customVariants.map(v => `<code class="doc-page__signature-variant">${escapeHtml(v)}</code>`).join('\n')
}
