# Dvala Effects — Implementation Plan
...
### 0a. Define the `DvalaValue` serialization contract ✅ DONE
...
Write a `isSerializable(value: DvalaValue): boolean` validator used at suspension time to
...
```typescript
type Step =
  | { type: 'Value';   value: DvalaValue }
  | { type: 'Eval';    node: AstNode; env: Context; k: Frame[] }
  | { type: 'Apply';   frame: Frame; value: DvalaValue; k: Frame[] }
  | { type: 'Perform'; effect: EffectRef; args: DvalaValue[]; k: Frame[] }
```
...
- Uses `Any` (Dvala value type), `ContinuationStack` (from frames.ts), `ContextStack`, `EffectRef`, and `AstNode`
...
- `src/evaluator/trampoline.ts` (~2100 lines) — full explicit-stack evaluator with:
  ...
  - **Helper functions**: `dispatchCall`, `dispatchFunction`, `dispatchDvalaFunction`, `setupUserDefinedCall`, `wrapMaybePromiseAsStep`, plus for-loop helpers (`processForLetBindings`, `processForGuards`, `processForNextLevel`, `advanceForElement`).
...
function runSyncTrampoline(initial: Step): DvalaValue {
  let step = initial
  while (true) {
    const next = tick(step)
    if (next instanceof Promise) throw new DvalaError('Async effect in runSync')
...
Stack overflow on deeply recursive Dvala programs is eliminated as a bonus.
...
- `src/evaluator/trampoline.ts` — Added trampoline loop functions:
  ...
  - **`runSyncTrampoline(initial)`**: Synchronous loop wrapper — iterates `tick()` calls, throws `DvalaError` if a `Promise` surfaces (async operation in sync context).
...
- `src/Dvala/Dvala.ts` — Import changed from `'../evaluator'` to `'../evaluator/trampoline'`, wiring all public API through the trampoline.
...
- Old recursive evaluator (`src/evaluator/index.ts`) still exists but is no longer imported by `Dvala.ts`. Will be deleted in a later cleanup step.
...
- Stack overflow no longer possible on deeply recursive Dvala programs
...
- `src/Dvala/Dvala.ts` — imports `evaluate`, `evaluateAsync`, `evaluateNode` from `'../evaluator/trampoline'`
...
Effects fully within Dvala code, no host API yet.
...
- `src/builtin/specialExpressions/effect.ts` — NEW: `EffectNode` type, docs with category `'special-expression'`, `getUndefinedSymbols` (returns empty set since effect names are not Dvala symbols)
...
type RunResult =
  ...
  | { type: 'suspended'; blob: string; meta?: DvalaValue }
...
interface SuspensionBlob {
  ...
  meta: DvalaValue      // from suspend(meta)
}
```
...
|---|---|---|
| `dvala.log` | `console.log(...args)`, resumes with `null` | ✅ |
| `dvala.now` | `Date.now()` | ✅ |
| `dvala.random` | `Math.random()` | ✅ |
| `dvala.sleep` | `setTimeout(resolve, ms)`, resumes with `null` | ❌ async only |

`dvala.prompt` deferred — environment-dependent, hosts provide their own.

Override in tests for determinism:

```typescript
handlers: {
  'dvala.now':    async ({ resume }) => resume(fixedTimestamp),
...
- `src/evaluator/standardEffects.ts` — NEW file (~100 lines):
  ...
  - 4 handlers: `dvala.log` (sync, console.log → null), `dvala.now` (sync, Date.now()), `dvala.random` (sync, Math.random()), `dvala.sleep` (async, setTimeout → null with argument validation)
  ...
  - Async effects (`dvala.sleep`) return `Promise<Step>` — only work in `run` (sync trampoline throws on Promise)
...
- `__tests__/effects.test.ts` — 22 new Phase 5 tests (109 total):
  - 5a: `dvala.log` — via run, via Dvala.run (sync), no args, host override, local try/with override
  - 5b: `dvala.now` — via run, sync, host override for determinism, local try/with override
  - 5c: `dvala.random` — via run, sync, host override for determinism, local try/with override
  - 5d: `dvala.sleep` — via run, throws in sync context, validates arguments (negative, non-number), host override
...
interface ParallelBlob {
  branches: Array<
    | { status: 'completed'; value: DvalaValue }
    | { status: 'suspended'; blob: string; meta: DvalaValue }
  >
...
- `SuspensionSignal` guard added in `tick()` catch block to prevent Dvala `try/catch` from intercepting suspension signals
...
### 7a. `dvala.debug.step` injection ✅ DONE

When a `dvala.debug.step` handler is registered, the trampoline injects a `perform(dvala.debug.step)`
between every evaluation step. No changes to Dvala source needed.
...
- `src/debug.ts` — `createDebugger()` factory, `DvalaDebugger` interface, `buildDebugAst`, `processResult`, `parseStepInfo`, `resumeFromBlob`
...
- Export via `@mojir/dvala/debug` in `package.json` and `rollup.config.js`
...
- Coverage: debug.ts at 95.88% statements, 100% functions (uncovered lines are defensive catch blocks for non-DvalaError errors that get wrapped in DvalaError by the handler dispatch before reaching these paths)
...
- **Rewrite as Dvala**: Implement `map`, `filter`, `reduce` etc. as Dvala standard library
