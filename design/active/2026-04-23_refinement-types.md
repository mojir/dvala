...
- **Trivial-collapse** in simplify: `simplifyRefinedType` handles `{x|true}` → base, `{x|false}` → Never, base-tautology → base, and disjoint-with-base → Never. Wired into `simplify.ts`'s `Refined` case after the merge step.
