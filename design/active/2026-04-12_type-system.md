...
| 25 | Generic upper bounds | `<T: U>` on type aliases and annotation-scoped function types | `type NonEmpty<T: Sequence> = ...` constrains `T` at declaration; `let f: <T: Number>(T) -> T = ...` provides annotation-scoped bounded polymorphism. Colon syntax (Kotlin/Rust convention), not `extends`. Multi-bound via existing intersection (`T: A & B`). Bounds propagate through Simple-sub's `Var.upperBounds` — inference unchanged. Phase 0a shipped 2026-04-24 as prerequisite for refinement types Phase 1. Phase 0b (let-binding-scoped `<T>` syntax) deferred. See `design/active/2026-04-24_upper-bounds.md`. |
