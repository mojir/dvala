...
| 25 | Generic upper bounds | `<T: U>` on type aliases, annotation-scoped function types, and let bindings | `type NonEmpty<T: Sequence> = ...` constrains `T` at declaration; `let f: <T: Number>(T) -> T = ...` provides annotation-scoped bounded polymorphism; `let f<T: Number> = (x: T) -> x` gives let-binding-scoped `T` visible through the entire RHS (including intermediate annotations). Colon syntax (Kotlin/Rust convention), not `extends`. Multi-bound via existing intersection (`T: A & B`). Bounds propagate through Simple-sub's `Var.upperBounds` — inference unchanged. Both Phase 0a and Phase 0b shipped 2026-04-24. See `design/active/2026-04-24_upper-bounds.md`. |
