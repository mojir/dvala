...
- Initial cut (Phase 2.5c, 2026-04-26): `if` + `Block` + early-return path-checked. `match` was conservatively rejected (treated as "did not prove P") to ship faster — sound but restrictive.
- Follow-up (2026-04-26): `match` proof-checking added to `terminalProves`/`statementGuarantees` mirroring the existing `If` handling. Every case's body must establish the predicate (each case is a possible normal-return path). Pattern-binding-aware substitution remains a future enhancement — assertions that reference a case-binding name (`case n then assert(n > 0)`) instead of the outer parameter name (`assert(x > 0)`) are still conservatively rejected.
