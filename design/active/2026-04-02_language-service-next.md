...
Cross-file rename shipped (direct + transitive re-exports); pick the next item from the "Remaining Features" section below based on available time.

Near-term options (any order):
...
The heavier items (code actions, call hierarchy) should wait until after the lighter ones ship, because they need more plumbing and the lighter ones may surface issues with the symbol table that affect design choices there.
...
- Cross-file rename (direct importers — PR #70, transitive re-exports — PR #72)
...
1. **Semantic tokens** — visual polish, straightforward mapping from symbol table
2. **Inlay hints** — nice complement to signature help
3. **Code actions** — highest effort but biggest productivity boost
4. **Selection range** — quality-of-life, moderate effort
5. **Call hierarchy** — most complex, needs call graph tracking
