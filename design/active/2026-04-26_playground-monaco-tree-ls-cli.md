...
- **Execution-semantics spike (Phase 0 step 5) — keep or drop.** The spike asks "is the playground's run path divergent from `dvala run`?" If a quick read of [playground-www/src/scripts.ts](../../playground-www/src/scripts.ts) and [cli/src/cli.ts](../../cli/src/cli.ts) entry points shows they already share `createDvala` + `allBuiltinModules`, the spike is moot and Phase 1 step 17 is a no-op. Decide via a 30-minute read before committing Phase 0 to the spike.
