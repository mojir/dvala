...
23j. Snapshot tab + view switcher. Clicking a snapshot in the Snapshots list opens the JSON file as an editor-area tab. Tab content has a header view-switcher: **UI** (custom snapshot inspector — source view at suspension point, scope tree, suspended computation, resume controls), **Tree** (interactive JSON tree), **Raw** (Monaco JSON with syntax highlighting). Default: UI. View choice persisted per-tab in localStorage. Tabs are read-only. **Implementation decisions (2026-04-30, captured via /interview):**
- **Tab content shape: kind discriminator on `OpenTab`.** Snapshots are the first non-text tab kind; the `OpenTab` union grows to `{ kind: 'file', … } | { kind: 'snapshot', … }`. The original "every tab has a Monaco model" simplification (Path A) was rejected once future non-text kinds (image preview, rendered markdown, etc.) were named — Monaco models can't represent binary content, so a kind discriminator is the right architectural seam to introduce now rather than retrofit later. The 23h "kind removal" was about *lifecycle* special-casing (sticky scratch); a content-type kind is a different category and not a regression.
- **View-switcher placement: `dvala-panel-header`.** The existing per-tab chrome surface (where file title, locked indicator, and snapshot breadcrumbs already live) gains a view-switcher slot shown only when the active tab is a snapshot. No second header inside the tab body; chrome-vs-content separation stays consistent across tab kinds.
- **Modal-removal scope: 23j flips entry points, 23l deletes the carcass.** Every user-visible entry point (side-panel clicks, output-link clicks, anywhere else) routes to the new tab in 23j; the modal *implementation* survives as unreachable dead code until 23l sweeps it alongside the other legacy modals (tokenize / parse / AST / CST). Atomic user-visible swap; deletion grouped with other modal retirements per the design-doc framing of 23l.
- **URL forms dropped: both `?snapshotId=` and `?snapshot=<blob>` retired.** The former was redundant with tab persistence (snapshot tabs already persist in `open-tabs` + `active-tab-key`); the latter was fundamentally limited by URL length and the codebase already steered users toward Download (toast: "too large to share as a URL"). `shareCheckpoint()` / `shareSnapshot()` exports + the `MAX_URL_LENGTH` grey-out logic come out with them. Old URLs containing either param degrade gracefully (param ignored).
- **Action placement: tab toolbar = Resume only.** The snapshot tab is a *viewer + resumer*; everything else (Lock, Delete, Save-from-terminal, Download, Open) stays on the side-panel item action menu where it already lives. Matches "edit in tab, manage in explorer" IDE convention; avoids chrome duplication.
