...
**Fix.** Resolve engine imports to source so HMR covers the engine end-to-end:

- Vite alias in `playground-www/vite.config.ts` mapping the engine import path to `src/full.ts` (or equivalent entry point). Alternatively, use `vite-tsconfig-paths` to consume the engine via TS path mappings.
- Verify HMR end-to-end: editing a file under `src/` reflects in the playground without a manual rebuild. Stack traces in the playground should also point at engine source rather than bundled output.
...
- Port the existing tokenizer to a `monaco.languages.TokensProvider` backed by `tokenScan` — single source of truth across highlighter, parser, and LS. Add line-resumable state to `tokenScan` if needed for Monaco's incremental per-line tokenization.
...
**Architecture note.** The LS runs in a Web Worker from day one. The protocol is reused from the VSCode extension's out-of-process LS — Monaco providers in the main thread, LS logic in the worker, message-passing between them. Same discipline (serializable I/O, no shared object identity) the shared-LS extraction track demands anyway.
...
- ~~**Token provider strategy.**~~ **Decided 2026-04-26: `TokensProvider` backed by `tokenScan`.** Single source of truth between highlighter, parser, and language service. Risk to watch: `tokenScan` may need line-resumable state for Monaco's incremental per-line tokenization; if perf becomes a problem, address it then rather than preemptively falling back to Monarch.
