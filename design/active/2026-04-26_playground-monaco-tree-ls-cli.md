...
23i. Snapshots as JSON files. Shift snapshot persistence to `.dvala-playground/snapshots/<id>.json` (timestamp-based id with disambiguator suffix). The Snapshots left-panel tab becomes a curated list reading from this folder. Existing in-memory snapshot UI retired.
