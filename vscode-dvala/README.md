...
- **Special expressions**: `if`, `let`, `for`, `match`, `loop`, `block`, `import`, `effect`, `perform`, `parallel`, `race`, `recur`
- **Reserved words**: `do`, `else`, `case`, `in`, `when`, `while`, `as`, `then`, `end`, `with`, `_`, `true`, `false`, `null`
