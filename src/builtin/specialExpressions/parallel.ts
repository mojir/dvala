...
const docs: CustomDocs = {
  ...
  customVariants: ['parallel(fns)'],
  details: [
    [
      'fns',
      'array',
      'Array of zero-argument functions to evaluate concurrently. Typically `[-> perform(...), -> perform(...)]`.',
    ],
  ],
  description:
    'Evaluates all branch functions concurrently and returns an array of results in order. ' +
  ...
  examples: [{ code: 'parallel([-> 1 + 2, -> 3 + 4])', noRun: true }],
}
