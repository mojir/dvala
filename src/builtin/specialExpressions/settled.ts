...
const docs: CustomDocs = {
  ...
  customVariants: ['settled(fns)'],
  details: [
    [
      'fns',
      'array',
      'Array of zero-argument functions to evaluate concurrently. Each result is wrapped as `[:ok, value]` on success or `[:error, errorPayload]` on error.',
    ],
  ],
  description:
    'Evaluates all branch functions concurrently and returns an array of tagged results in order. ' +
  ...
  examples: [{ code: 'settled([-> perform(@host.fetch, url1), -> perform(@host.fetch, url2)])', noRun: true }],
}
