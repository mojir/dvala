...
type RaceNode = [typeof NodeTypes.Race, AstNode, number]

const docs: CustomDocs = {
  category: 'special-expression',
  customVariants: ['race(fns)'],
  details: [
    [
      'fns',
      'array',
      'Array of zero-argument functions to race concurrently. Typically `[-> perform(...), -> perform(...)]`.',
    ],
  ],
  description:
    'Races all branch functions concurrently. The first branch to complete wins — ' +
  ...
  examples: [{ code: 'race([-> perform(@a.get), -> perform(@b.get)])', noRun: true }],
}
