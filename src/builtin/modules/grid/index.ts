...
import { assertGrid, assertVector } from '@mojir/dvala-types'
import { assertArray } from '@mojir/dvala-types'
import { assertAny } from '@mojir/dvala-types'
...
function toAnyGrid(val: unknown): Any {
  return val as Any
}
    ...
    evaluate: (params, sourceCodeInfo): Any => {
      ...
      if (flatTable.length % rows !== 0) {
        throw new RuntimeError(
          `The number of elements in the grid must be divisible by rows, but got ${flatTable.length} and ${rows}`,
          sourceCodeInfo,
        )
      }
    ...
    evaluate: (params, sourceCodeInfo): Any => {
      ...
      switch (t) {
        case 1: // 90 degrees clockwise
          result = Array<Any>(width)
            .fill(null)
            .map(() => Array<Any>(height).fill(null))
        ...
        case 2: // 180 degrees
          result = Array<Any>(height)
            .fill(null)
            .map(() => Array<Any>(width).fill(null))
          for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
          ...
          ? (() => {
              ...
              rows.every(row => {
                if (grid[0]!.length !== (row as unknown[]).length) {
                  throw new RuntimeError(
                    `All rows must have the same length as the number of columns in grid, but got ${(row as unknown[]).length}`,
                    sourceCodeInfo,
                  )
                }
                return true
          ...
          ? (() => {
              ...
              cols.every(row => {
                if (trMatrix[0]!.length !== (row as unknown[]).length) {
                  throw new RuntimeError(
                    `All rows must have the same length as the number of rows in grid, but got ${(row as unknown[]).length}`,
                    sourceCodeInfo,
                  )
                }
                return true
    ...
    evaluate: (params, sourceCodeInfo): Any => {
      ...
      paramsArr.slice(1).every(grid => {
        if (grid[0]!.length !== cols) {
          throw new RuntimeError(
            `All grids must have the same number of columns, but got ${cols} and ${grid[0]!.length}`,
            sourceCodeInfo,
          )
        }
        return true
  ...
  concatCols: {
    evaluate: (params, sourceCodeInfo): Any => {
      ...
      paramsArr.slice(1).every(grid => {
        if (grid.length !== rows) {
          throw new RuntimeError(
            `All grids must have the same number of rows, but got ${rows} and ${grid.length}`,
            sourceCodeInfo,
          )
        }
        return true
  ...
  pushRows: {
    evaluate: (params, sourceCodeInfo): Any => {
      ...
      if (grid[0]!.length !== rows[0]!.length) {
        throw new RuntimeError(
          `All rows must have the same length as the number of columns in grid, but got ${grid[0]!.length} and ${rows[0]!.length}`,
          sourceCodeInfo,
        )
      }
  ...
  pushCols: {
    evaluate: (params, sourceCodeInfo): Any => {
      ...
      if (grid.length !== cols[0]!.length) {
        throw new RuntimeError(
          `All columns must have the same length as the number of rows in grid, but got ${cols.length}`,
          sourceCodeInfo,
        )
      }
  ...
  fromArray: {
    evaluate: (params, sourceCodeInfo): Any => {
      ...
      if (array.size % rows !== 0) {
        throw new RuntimeError(
          `The number of elements in the array must be divisible by rows, but got ${array.size} and ${rows}`,
          sourceCodeInfo,
        )
      }
...
for (const [key, docs] of Object.entries(moduleDocs)) {
  ...
  /* v8 ignore next 2 */
  if (gridFunctions[key]) gridFunctions[key].docs = docs
}
