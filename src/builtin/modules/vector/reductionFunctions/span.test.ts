...
describe('span', () => {
  ...
  it('should calculate the moving span of a vector', () => {
    expect(runVec('movingSpan([1, 2, 4, 7, 11, 16], 4)')).toEqual([6, 9, 12])
    expect(runVec('movingSpan([1, 2, 4, 7, 11, 16], 5)')).toEqual([10, 14])
    expect(runVec('movingSpan([1, 2, 4, 7, 11, 16], 6)')).toEqual([15])
    expect(runVec('movingSpan([], 0)')).toEqual([])
  })
  it('should calculate the centered moving span of a vector with padding', () => {
    expect(runVec('centeredMovingSpan([1, 2, 4, 7, 11], 4)')).toEqual([null, null, 6, 9, null])
    expect(runVec('centeredMovingSpan([], 0, 0, 100)')).toEqual([])
  })
  it('should calculate the running span of a vector', () => {
    expect(runVec('runningSpan([1, 2, 3, 4, 5, 6])')).toEqual([0, 1, 2, 3, 4, 5])
    expect(runVec('runningSpan([-1, -2, -3])')).toEqual([0, 1, 2])
    expect(runVec('runningSpan([])')).toEqual([])
  })
})
