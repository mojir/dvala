...
describe('medad', () => {
  ...
  it('should calculate the moving median absolute deviation of a vector', () => {
    ...
    expect(() => runVec('moving-medad([1], 100)')).toThrow(DvalaError)
    expect(() => runVec('moving-medad([], 1)')).toThrowError(DvalaError)
  })
  it('should calculate the centered moving median absolute deviation of a vector with padding', () => {
    ...
    expect(() => runVec('centered-moving-medad([1], 100)')).toThrow(DvalaError)
    expect(() => runVec('centered-moving-medad([], 1)')).toThrowError(DvalaError)
  })
  it('should calculate the running median absolute deviation of a vector', () => {
    ...
    expect(() => runVec('running-medad([])')).toThrowError(DvalaError)
  })
})
