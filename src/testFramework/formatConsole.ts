 ...
 */
export function formatConsole(result: TestRunResult, options?: { verbose?: boolean; color?: boolean }): { text: string; success: boolean } {
  ...
  const duration = result.durationMs !== undefined ? ` ${c.gray}(${(result.durationMs / 1000).toFixed(3)}s)${c.reset}` : ''
  ...
  const parts: string[] = []
  if (passed > 0)
    parts.push(`${c.green}${c.bold}${passed} passed${c.reset}`)
  if (failed > 0)
    parts.push(`${c.red}${c.bold}${failed} failed${c.reset}`)
  if (skipped > 0)
    parts.push(`${c.yellow}${skipped} skipped${c.reset}`)
