...
import { fromJS, toJS } from '@mojir/dvala-engine'
...
export function getFsModule(): DvalaModule {
  ...
  return {
    ...
    functions: {
      ...
      writeFile: jsFnToExpression(writeFile),
      writeJson: jsFnToExpression(writeJson),
      appendFile: jsFnToExpression(appendFile),
      exists: jsFnToExpression(exists),
      isFile: jsFnToExpression(isFile),
      isDirectory: jsFnToExpression(isDirectory),
      listDir: jsFnToExpression(listDir),
      mkdir: jsFnToExpression(mkdir),
      remove: jsFnToExpression(remove),
      copy: jsFnToExpression(copy),
      move: jsFnToExpression(move),
      getStats: jsFnToExpression(stats),
      chmod: jsFnToExpression(chmod),
      touch: jsFnToExpression(touch),
      createTempFile: jsFnToExpression(createTempFile),
      createTempDir: jsFnToExpression(createTempDir),
    },
  }
}
