...
const astFunctions: BuiltinNormalExpressions = {
  ...
  // --- Pretty printing ---
  prettyPrint: {
    evaluate: ([node], sourceCodeInfo, contextStack): string => {
      assertAstNode(node, sourceCodeInfo)
      const prettyPrint = contextStack.prettyPrint
      if (!prettyPrint) {
        throw new TypeError('Host did not provide a prettyPrint capability', sourceCodeInfo)
      }
