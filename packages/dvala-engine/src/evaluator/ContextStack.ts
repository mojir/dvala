...
import type { Context, LookUpResult, ParseSource, PrettyPrint } from './interface'
...
export interface CreateContextStackOptions {
...
export function createContextStack(options: CreateContextStackOptions = {}): ContextStack {
  const globalContext = options.globalContext ?? {}
  // Contexts are checked from left to right
  const contexts = options.contexts ? [globalContext, ...options.contexts] : [globalContext]
  ...
  return options.globalModuleScope ? contextStack : contextStack.create({})
}
