...
Dvala takes a different approach. The program above is just a program:
...
In Dvala, any interaction with the outside world — calling an LLM, reading a clock, asking a human — is an *effect*. Effects are declared as first-class values and invoked via `perform`:
...
let llm     = effect(llm.complete)
let approve = effect(com.myco.human.approve)
let log     = effect(dvala.log)              // standard effect — always available
```
...
```typescript
import { runSync, run, resume } from '@mojir/dvala'
...
interface EffectContext {
  args:    DvalaValue[]          // arguments from perform()
...
```
effect(dvala.log)               →  handlers key: 'dvala.log'
effect(llm.complete)           →  handlers key: 'llm.complete'
effect(com.myco.human.approve) →  handlers key: 'com.myco.human.approve'
```

Use `dvala.*` for standard library effects (default implementations provided). Use reverse-domain notation for domain-specific effects.

---

## Parallel & Race

Dvala supports concurrent effects natively. The handler contract is identical to single `perform` — the runtime handles coordination.
...
Dvala effects can pause across processes, machines, and time. The Dvala program looks synchronous — the host decides what "pausing" means.
...
Dvala effects make orchestration a language primitive. The same `perform` that calls an LLM can also wait for human input — the program doesn't change, only the handler does.
...
| Effect | Default | Use |
|---|---|---|
| `dvala.log` | `console.log` | Logging |
| `dvala.now` | `Date.now()` | Current timestamp |
...
Registering a `dvala.debug.step` handler activates debug mode. The runtime injects this effect between every operation — no changes to Dvala source needed.
