...
describe('runSync with sync effect handlers', () => {
  describe('basic resume', () => {
    it('resumes with a value from a sync handler', () => {
      const result = dvala.run('perform(@my.ask)', {
        effectHandlers: [{ pattern: 'my.ask', handler: ({ resume }) => resume(42) }],
      })
    ...
    it('resume value is used as result of perform expression', () => {
      ...
      `,
        {
          effectHandlers: [{ pattern: 'my.val', handler: ({ resume }) => resume(10) }],
        },
      )
    ...
    it('works with multiple perform calls', () => {
      ...
      `,
        {
          effectHandlers: [{ pattern: 'my.val', handler: ({ arg, resume }) => resume(arg ?? null) }],
        },
      )
  ...
  describe('fail', () => {
    it('fail() causes an error to be thrown', () => {
      expect(() =>
        dvala.run('perform(@my.bad)', {
          effectHandlers: [{ pattern: 'my.bad', handler: ({ fail }) => fail('something went wrong') }],
        }),
    ...
    it('fail() with no message throws a generic error', () => {
      expect(() =>
        dvala.run('perform(@my.bad)', {
          effectHandlers: [{ pattern: 'my.bad', handler: ({ fail }) => fail() }],
        }),
      ).toThrow()
    })
  })
  ...
  describe('next', () => {
    ...
    it('next() with no further handler throws unhandled effect', () => {
      expect(() =>
        dvala.run('perform(@my.effect)', {
          effectHandlers: [{ pattern: 'my.effect', handler: ({ next }) => next() }],
        }),
      ).toThrow()
    })
  })
  ...
  describe('pattern matching', () => {
    it('wildcard * matches any effect', () => {
      const result = dvala.run('perform(@any.effect)', {
        effectHandlers: [{ pattern: '*', handler: ({ resume }) => resume('caught') }],
      })
    ...
    it('node.* matches the node itself and descendants', () => {
      const results = [
        dvala.run('perform(@my)', {
          effectHandlers: [{ pattern: 'my.*', handler: ({ resume }) => resume(1) }],
        }),
        dvala.run('perform(@my.child)', {
          effectHandlers: [{ pattern: 'my.*', handler: ({ resume }) => resume(2) }],
        }),
        dvala.run('perform(@my.child.deep)', {
          effectHandlers: [{ pattern: 'my.*', handler: ({ resume }) => resume(3) }],
        }),
      ]
  ...
  describe('error cases', () => {
    it('throws on unhandled effect', () => {
      expect(() => dvala.run('perform(@my.unhandled)', { effectHandlers: [] })).toThrow()
    })
  ...
  describe('local handlers take priority over sync host handlers', () => {
    it('local handler wins over effectHandlers', () => {
      ...
      `,
        {
          effectHandlers: [{ pattern: 'my.effect', handler: ({ resume }) => resume('host') }],
        },
      )
