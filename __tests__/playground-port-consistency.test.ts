...
// `port: <number>` setting in `apps/playground-www/vite.config.mjs`. This test
...
describe('playground dev port consistency', () => {
  const viteConfig = readFileSync('apps/playground-www/vite.config.mjs', 'utf-8')
  const match = /\bport:\s*(\d+)/.exec(viteConfig)
  if (match === null) throw new Error('apps/playground-www/vite.config.mjs must specify a numeric `port:` setting')
