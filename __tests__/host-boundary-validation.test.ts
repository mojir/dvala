...
import type { Handlers, RunResult } from '@mojir/dvala-engine'
...
describe('host boundary validation', () => {
  ...
  describe('scope bindings', () => {
    ...
    it('should reject undefined in scope', () => {
      expect(() => dvala.run('x', { scope: { x: undefined } })).toThrow(TypeError)
      expect(() => dvala.run('x', { scope: { x: undefined } })).toThrow(/scope binding "x".*undefined/)
    })

    it('should reject function in scope', () => {
      expect(() => dvala.run('x', { scope: { x: () => 1 } })).toThrow(TypeError)
      expect(() => dvala.run('x', { scope: { x: () => 1 } })).toThrow(/scope binding "x".*JS functions/)
    })

    it('should reject nested invalid values in scope', () => {
      expect(() => dvala.run('x', { scope: { x: { nested: undefined } } })).toThrow(
        /scope binding "x".*undefined.*at \.nested/,
      )
    })

    it('should reject Date in scope', () => {
      expect(() => dvala.run('x', { scope: { x: new Date() } })).toThrow(/scope binding "x".*Date/)
    })
    ...
    it('should reject class instance in scope', () => {
      ...
      expect(() => dvala.run('x', { scope: { x: new Foo() } })).toThrow(/scope binding "x".*Class instance \(Foo\)/)
    })
  })
  ...
  describe('async resume() with promise', () => {
    ...
    it('should reject invalid resolved value from promise', async () => {
      const handlers: Handlers = [
        {
          pattern: 'test.fetch',
          handler: ctx => {
            ctx.resume(Promise.resolve(undefined))
          },
        },
      ]
      const result = await dvala.runAsync('perform(@test.fetch)', {
      ...
      expect(result.type).toBe('error')
      if (result.type === 'error') expect(result.error.message).toMatch(/resume\(\).*undefined/)
    })
  })
  ...
  describe('resumeFrom() in effect handler', () => {
    ...
    it('should reject invalid resumeFrom value', async () => {
      ...
      expect(result.type).toBe('error')
      if (result.type === 'error') expect(result.error.message).toMatch(/resumeFrom\(\).*undefined/)
    })
  })
  ...
  describe('suspend() meta validation', () => {
    ...
    it('should reject function in suspend meta', async () => {
      ...
      expect(result.type).toBe('error')
      if (result.type === 'error') expect(result.error.message).toMatch(/suspend\(\) meta.*JS functions/)
    })
  })
  ...
  describe('checkpoint() meta validation', () => {
    ...
    it('should reject function in checkpoint meta', async () => {
      ...
      expect(result.type).toBe('error')
      if (result.type === 'error') expect(result.error.message).toMatch(/checkpoint\(\) meta.*JS functions/)
    })
  })
  ...
  describe('standalone resume() scope validation', () => {
    it('should reject invalid scope values in resume', async () => {
      ...
      expect(result.type).toBe('error')
      if (result.type === 'error') expect(result.error.message).toMatch(/scope binding "bad".*undefined/)
    })
  })
})
