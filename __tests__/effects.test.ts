...
describe('phase 2 — Local Effect Handling', () => {
  describe('2a: @name special expression', () => {
    it('should return an effect reference', () => {
      const result = dvala.run('@dvala.io.println')
  ...
  describe('2c: TryWithFrame handler dispatch', () => {
    ...
    it('should delegate to outer handle/with when no local match', () => {
      const result = dvala.run(`
        handle
          handle
    ...
    it('should nest handle/with blocks correctly', () => {
      const result = dvala.run(`
        handle
          let a = handle
            perform(@inner, "a")
...
describe('phase 3 — Host Async API', () => {
  ...
  describe('3b: local handlers take precedence over host handlers', () => {
    it('should use local handle/with handler instead of host handler', async () => {
      const result = await dvala.runAsync(`
        handle
          perform(@my.eff, "test")
...
describe('phase 4 — Suspension & Resume', () => {
  ...
  describe('4a-checkpoint: dvala.checkpoint effect', () => {
    ...
    it('should always capture a snapshot even when a local handle...with handler intercepts', async () => {
      let capturedSnapshots: readonly unknown[] = []
      const result = await dvala.runAsync(`
        let x = handle
          perform(@dvala.checkpoint, "cp")
  ...
  describe('4d: end-to-end suspension workflow', () => {
    ...
    it('should work with local handle/with handlers after resume', async () => {
      const r1 = await dvala.runAsync(`
        let x = perform(@my.wait);
        handle
          perform(@my.local, x)
...
describe('phase 5 — Standard Effects', () => {
  describe('5a: dvala.io.println', () => {
    it('should write to stdout and return value (via Dvala.run)', () => {
      ...
      try {
        const result = dvala.run('perform(@dvala.io.println, "test")')
    ...
    it('should be overridable by local handle/with', () => {
      const result = dvala.run(`
        handle
  ...
  describe('5e: standard effects in workflows', () => {
    it('should use multiple standard effects in sequence', async () => {
      ...
      try {
        const result = await dvala.runAsync(`
          do
            perform(@dvala.io.println, "Starting");
            let t = perform(@dvala.time.now);
            let r = perform(@dvala.random);
            perform(@dvala.io.println, "Done");
    ...
    it('should work with standard effects + suspension', async () => {
      ...
      try {
        const r1 = await dvala.runAsync(`
          do
            perform(@dvala.io.println, "Before suspend");
            let input = perform(@my.wait);
            perform(@dvala.io.println, "After resume: " ++ input);
            input
          end
        `, {
          effectHandlers: [
    ...
    it('should work with runSync for sync standard effects', () => {
      ...
      try {
        const result = dvala.run(`
          do
            perform(@dvala.io.println, "sync log");
            let t = perform(@dvala.time.now);
            let r = perform(@dvala.random);
            number?(t) && number?(r)
          end
...
describe('step 1 — handle...with...end', () => {
  describe('basic handle...with handler', () => {
    ...
    it('should skip handle...with frame on success (no effect performed)', () => {
      const result = dvala.run(`
        handle
          1 + 2
  ...
  describe('handle...with and body scoping', () => {
...
describe('step 9 — effect-name accessor', () => {
  ...
  it('should work with effect stored in variable', () => {
    expect(dvala.run('let e = @dvala.io.println; effect-name(e)')).toBe('dvala.io.println')
  })
...
describe('step 10 — predicate-based handler matching', () => {
  it('should match with a predicate function in handler', () => {
    const result = dvala.run(`
      handle
        perform(@dvala.io.println, "hello")
  ...
  it('should mix exact-match and predicate checks', () => {
    const result = dvala.run(`
      handle
        perform(@dvala.io.println, "hello")
      with [(eff, arg, nxt) ->
...
describe('host handler wildcard patterns', () => {
  ...
  describe('effectName on context', () => {
    ...
    it('effectName is correct for wildcard suffix handlers', async () => {
      let capturedName = ''
      await dvala.runAsync('perform(@dvala.io.println, "hello")', {
        effectHandlers: [
