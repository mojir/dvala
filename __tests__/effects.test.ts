...
import { qualifiedNameMatchesPattern, findMatchingHandlers, generateUUID } from '@mojir/dvala-engine'
...
describe('phase 3 — Host Async API', () => {
  ...
  describe('3d: end-to-end integration', () => {
    it('should run a multi-step effect workflow', async () => {
      ...
      expect(log).toEqual(['Summarize this doc', 'Critique: [result for: Summarize this doc]'])
    })
      ...
      `,
        {
          effectHandlers: [
            ...
            {
              pattern: 'com.myco.human.approve',
              handler: async ({ resume: doResume }) => {
...
describe('phase 4 — Suspension & Resume', () => {
      ...
      `,
        {
          effectHandlers: [
            {
              pattern: 'my.check',
              handler: async ({ checkpoint, resume: r }) => {
      ...
      `,
        {
          effectHandlers: [
            {
              pattern: 'my.check',
              handler: async ({ resumeFrom, resume: r }) => {
      ...
      `,
        {
          effectHandlers: [
            {
              pattern: 'my.action',
              handler: async ({ snapshots, resumeFrom, resume: r }) => {
                callCount++
                if (callCount === 1) {
      ...
      `,
        {
          effectHandlers: [
            {
              pattern: 'my.action',
              handler: async ({ resume: r, resumeFrom }) => {
                r(42)
                try {
                  const fakeSnapshot = {
                    id: 'fake-id',
      ...
      `,
        {
          effectHandlers: [
            {
              pattern: 'llm.complete',
              handler: async ({ arg, resume: r, checkpoint }) => {
        ...
        handlers: [
          {
            pattern: 'dvala.io.print',
            handler: async ({ arg, resume: r }) => {
              replayOutputs.push(arg as string)
              r(null)
            },
          },
        ],
  ...
  describe('4b: resume() API', () => {
    ...
    it('should resume with an object value', async () => {
      ...
      const r2 = await resumeContinuation(
        r1.snapshot,
        PersistentMap.fromRecord({ approved: false, reason: 'Budget exceeded' }),
      )
    ...
    it('should return error for invalid continuation object', async () => {
      const result = await resumeContinuation(
        {
          id: 'test-id',
          continuation: { version: 1, k: 'bad', contextStacks: [] },
          timestamp: 0,
          index: 0,
          executionId: 'test',
          message: 'test',
    ...
    it('should return error for wrong version', async () => {
      const result = await resumeContinuation(
        {
          id: 'test-id',
          continuation: { version: 999, k: [], contextStacks: [] },
          timestamp: 0,
          index: 0,
          executionId: 'test',
          message: 'test',
  ...
  describe('4d: end-to-end suspension workflow', () => {
    it('should complete a full suspend-store-resume cycle', async () => {
      ...
      const r2 = await resumeContinuation(storedSnapshot, PersistentMap.fromRecord({ approved: true, reason: null }), {
        handlers,
      })
      expect(r2.type).toBe('completed')
      if (r2.type === 'completed') {
...
describe('phase 5 — Standard Effects', () => {
  ...
  describe('5b: dvala.time.now', () => {
    ...
    it('should be overridable by host handler for determinism', async () => {
      ...
      const result = await dvala.runAsync('perform(@dvala.time.now)', {
        effectHandlers: [{ pattern: 'dvala.time.now', handler: async ({ resume: r }) => r(fixedTime) }],
      })
  ...
  describe('5c: dvala.random', () => {
    ...
    it('should be overridable by host handler for determinism', async () => {
      const result = await dvala.runAsync('perform(@dvala.random)', {
        effectHandlers: [{ pattern: 'dvala.random', handler: async ({ resume: r }) => r(0.42) }],
      })
  ...
  describe('5d: dvala.sleep', () => {
    ...
    it('should throw in sync context', () => {
      expect(() => dvala.run('perform(@dvala.sleep, 10)')).toThrow()
    })
  ...
  describe('6a: parallel(...expressions)', () => {
    ...
    describe('parallel suspension', () => {
        ...
        `,
          {
            effectHandlers: [
              {
                pattern: 'fast.op',
                handler: async ({ resume: res }) => {
                  res('fast-result')
                },
              },

              {
                pattern: 'needs.approval',
                handler: async ({ suspend }) => {
      ...
      it('should resume suspended parallel and complete', async () => {
        ...
        if (result1.type !== 'suspended') return
      ...
      it('should handle multiple suspended branches one at a time', async () => {
        ...
        if (result3.type !== 'suspended') return
      ...
      `,
        {
          effectHandlers: [
            ...
            {
              pattern: 'slow.op',
              handler: async ({ signal, resume: res }) => {
      ...
      `,
        {
          effectHandlers: [
...

            {
              pattern: 'err.op',
              handler: async ({ resume: res }) => {
                res(Promise.reject(new Error('handler error')))
              },
            },
          ],
        },
        ...
        effectHandlers: [
          {
            pattern: 'dvala.io.print',
            handler: async () => {
              throw new Error('async host error')
            },
          },
        ],
      },
    ...
    it('wildcard suffix does NOT match without dot boundary', async () => {
      ...
      const result = await dvala.runAsync('perform(@dvalaXXX, "val")', {
        effectHandlers: [
          {
            pattern: 'dvala.*',
            handler: async ({ effectName, arg, resume }) => {
    ...
    it('effectName is correct for wildcard suffix handlers', async () => {
      ...
      await dvala.runAsync('perform(@dvala.io.print, "hello")', {
        effectHandlers: [
          {
            pattern: 'dvala.*',
            handler: async ({ effectName, resume }) => {
              capturedName = effectName
              resume(null)
            },
          },
