...
describe('array functions', () => {
  ...
  describe('flatten', () => {
    ...
    it('immutability', () => {
      dvala.run('flatten(nestedArray)', { scope: testData as unknown as Record<string, unknown> })
    })
  })
  ...
  describe('mapcat', () => {
    it('samples', () => {
      expect(
        dvala.run('let { mapcat } = import("sequence"); mapcat([[3, 2, 1, 0], [6, 5, 4], [9, 8, 7]], reverse)'),
      ...
      expect(
        dvala.run(
          'let { mapcat } = import("sequence"); let foo = (n) -> do [-(n, 1), n, +(n, 1)] end; mapcat([1, 2, 3], foo)',
        ),
      ).toEqual([0, 1, 2, 1, 2, 3, 2, 3, 4])
      expect(
        dvala.run('let { mapcat } = import("sequence"); mapcat([[1, 2], [2, 2], [2, 3]], -> $ filter isOdd)'),
      ).toEqual([1, 3])
    })
  })
  ...
  describe('runningFn', () => {
    it('samples', () => {
      expect(dvala.run('let { runningFn } = import("vector"); runningFn([1, 2, 3], sum)')).toEqual([1, 3, 6])
      expect(() => dvala.run('let { runningFn } = import("vector"); runningFn(1)')).toThrow(DvalaError)
      expect(() => dvala.run('let { runningFn } = import("vector"); runningFn(1, sum)')).toThrow(DvalaError)
      expect(() => dvala.run('let { runningFn } = import("vector"); runningFn(1, sum, null)')).toThrow(DvalaError)
    })
  })
  describe('movingFn', () => {
    it('samples', () => {
      expect(dvala.run('let { movingFn } = import("vector"); movingFn([1, 2, 3], 2, sum)')).toEqual([3, 5])
      expect(dvala.run('let { movingFn } = import("vector"); movingFn([1, 2, 3], 1, sum)')).toEqual([1, 2, 3])
      expect(dvala.run('let { movingFn } = import("vector"); movingFn([1, 2, 3], 3, sum)')).toEqual([6])
      expect(() => dvala.run('let { movingFn } = import("vector"); movingFn([1, 2, 3], 4, sum)')).toThrow(DvalaError)
