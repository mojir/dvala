  ...
  for (const dvala of [createDvala(), createDvala({ debug: true })]) {
    describe('string as function', () => {
      it('samples', () => {
        ...
        expect(() => dvala.run('"x"({ firstName: "Albert", lastName: "Mojir" })')).toThrow(KeyError)
        expect(dvala.run('"Albert"(2)')).toBe('b')
        expect(() => dvala.run('"Albert"(12)')).toThrow(KeyError)
    ...
    describe('object as function', () => {
      it('samples', () => {
        ...
        expect(() => dvala.run('{ firstName: "Albert", lastName: "Mojir" }("x")')).toThrow(KeyError)
        expect(() => dvala.run('{ firstName: "Albert", lastName: "Mojir" }()')).toThrow(DvalaError)
    ...
    describe('array as function', () => {
      it('samples', () => {
        ...
        expect(() => dvala.run('"Albert"(10)')).toThrow(KeyError)
    ...
    describe('number as function', () => {
      it('samples', () => {
        ...
        expect(() => dvala.run('3(["Albert", "Mojir"])')).toThrow(KeyError)
        expect(dvala.run('1(push([1], 2, 3))')).toBe(2)
        expect(dvala.run('1("Albert")')).toBe('l')
        expect(() => dvala.run('10("Albert")')).toThrow(KeyError)
