...
let matrixMultiply = (matrixA, matrixB) -> do
  ...
  unless every?(matrixA, row -> array?(row) && count(row) == colsA) then perform(@dvala.error, "First matrix has inconsistent row lengths") end;
  unless every?(matrixB, row -> array?(row) && count(row) == colsB) then perform(@dvala.error, "Second matrix has inconsistent row lengths") end;

  // Check if matrices can be multiplied
  unless colsA == rowsB then perform(@dvala.error, "Matrix dimensions mismatch: first matrix columns must equal second matrix rows") end;
