...
describe('retrigger()', () => {
    ...
    `,
      {
        effectHandlers: [
          {
            pattern: 'foo.bar',
            handler: async ({ arg, resume: r, suspend }) => {
              if (arg === 'A') r('resumed-A')
              else suspend()
            },
          },
        ],
    ...
    `,
      {
        effectHandlers: [
          {
            pattern: 'foo.bar',
            handler: async ({ arg, resume: r, suspend, signal }) => {
              if (arg === 'A') {
                r('got-A')
              } else if (arg === 'B') {
                suspend()
  ...
  it('re-suspends when the re-triggered branch suspends again and other branches abort', async () => {
    ...
    const r1 = await dvala.runAsync(
      ...
      parallel([-> perform(@foo.bar, "A"), -> perform(@foo.bar, "B"), -> perform(@foo.bar, "C")])
    `,
      { effectHandlers: [{ pattern: 'foo.bar', handler: suspendHandler }] },
    )
    ...
    const r2 = await retrigger(r1.snapshot, {
      handlers: [{ pattern: 'foo.bar', handler: suspendHandler }],
    })
    expect(r2.type).toBe('suspended')
  })
