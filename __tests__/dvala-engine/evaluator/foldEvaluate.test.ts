...
import { parse } from '@mojir/dvala/tooling'
import type { AstNode } from '@mojir/dvala-types'
import { minifyTokenStream } from '@mojir/dvala/tooling'
import { tokenize } from '@mojir/dvala/tooling'
import { toJS } from '@mojir/dvala-engine'
import { createContextStack } from '@mojir/dvala-engine'
import type { FoldResult } from '@mojir/dvala-engine'
...
function runFold(source: string): FoldResult {
  ...
  const result = evaluateNodeForFold(node, createContextStack({ modules: moduleMap }))
  if (result.ok) {
...
describe('evaluateNodeForFold — differential equivalence with normal evaluator', () => {
  ...
  it('reports the effect name for a direct top-level perform', () => {
    ...
    const fold = evaluateNodeForFold(
      ...
      createContextStack({ modules: moduleMap }),
    )
  ...
  it('fails silently (reason=error) when a free variable cannot be resolved', () => {
    ...
    const fold = evaluateNodeForFold(parseExpression('undefinedVar + 1'), createContextStack({ modules: moduleMap }))
